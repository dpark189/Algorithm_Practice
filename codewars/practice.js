...
function doneOrNot(board){
  //your code here
  if (!colCheck(board)) {return 'Try again!';};
  if (!regions(board)) return 'Try again!';
  for (let i = 0; i < board.length; i++){
    if(!rowCheck(board[i])) return 'Try again!';
  }
  return 'Finished!';
}

function rowCheck(arr) {
  const row = JSON.stringify(arr.sort());
  const ref = JSON.stringify([1,2,3,4,5,6,7,8,9]);
  return row === ref;
}

function transpose(board){
  const len = board[0].length;
  let tran = new Array(len);
  for (let i = 0; i < len; i++){
    tran[i] = new Array(len);
  }
  for(let i = 0; i < len; i++){
    for (let j = 0; j < len; j++){
      tran[j][i] = board[i][j];
...
function regions(board) {
  const len = board[0].length;
  let reg = new Array(len);
  for(let i =0; i < len; i++) {
    reg[i] = [];
  }
  for(let i = 0; i < len; i++){
    for(let j = 0; j < len; j++){
      const rowGroup = Math.floor(i/3);
      const colGroup = Math.floor(j/3) * 3;
      const group = colGroup + rowGroup;
      reg[group].push(board[i][j]);
    }
  }
  for (let i = 0; i < len; i++){
    if (!rowCheck(reg[i])) return false;
  }
  return true;
}
